...
For operational setup and workflow guidance (install, agent integrations, hooks,
and skills), see the project `README.md`.

Note: maintainers develop in `agents/` and ship installable artifacts from the
committed `outputs/` tree (generated by `./bin/cace-build`).
